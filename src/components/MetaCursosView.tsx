...
export default function MetaCursosView({
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    try {
      if (editingItem) {
        await updateDoc(doc(db, COLLECTIONS.META_CURSOS, editingItem.id), payload);
        onToast("Meta Curso atualizada com sucesso!");
        setEditingItem(null);
      } else {
        ...
        onToast("Meta Curso cadastrada com sucesso!");
      }
      setFormData({
        semestre: "",
        curso: "",
      ...
      console.error(error);
      onToast("Erro ao salvar Meta Curso.", "error");
    }
  };
        ...
        <div className="p-6 border-b border-slate-100 flex justify-between items-center bg-slate-50/50">
          <div>
            <h3 className="text-xl font-bold text-slate-900">
              Cadastro de Metas por Curso
            </h3>
            <p className="text-sm text-slate-500 mt-1">
              Cadastre e acompanhe o realizado de cada curso
            </p>
          </div>
        </div>
        <div className="p-6">
            ...
            <tbody className="divide-y divide-slate-100">
              {metaCursos.sort((a,b) => b.semestre.localeCompare(a.semestre) || a.curso.localeCompare(b.curso)).map((item) => (
                ...
                >
                  ...
                  <td className="px-6 py-4 whitespace-nowrap">
                    <span className="text-slate-600">{item.curso}</span>
                  </td>
                  <td className="px-6 py-4 whitespace-nowrap">
